...
function findAll(){
	UserModel.find({}, function(err, results){
		...
		if(results){
			console.log('select user document object #0 -> id : %s, name : %s', results[0]._doc.id, results[0]._doc.name);
		}
	});
}

function doTest(){
	var user = new UserModel({"info" : '1 1'});
...
function createUserSchema(){
	...
	UserSchema
		...
		.set(function(info){
			...
			console.log('virtual info setting : %s, %s', this.id, this.name);
		})
